<template>
  <div class="container">
    <h3 class="text-center">Audit Department</h3>
    <table class="table">
      <thead>
        <tr>
      ...
      <tbody>
        <tr v-for="student,key in students">
          ...
          <td v-if="student.bursarycleared==1">cleared</td>
          <td v-else>Uncleared</td>
          <td><a type="button" id="show-modal" @click="showModal=true;
            setVal(student.id, student.reg_no, student.name, student.bursarycleared)"
    ...
    let vm = this;
     ...
     makePagination(meta, links){
      ...
      updateStudent(){
        ...
        var bursarycleared = document.getElementById('status').value
        axios.put(`api/updateauditstatus/${id}`,
          {
            reg_no:reg_no,
            name:name,
