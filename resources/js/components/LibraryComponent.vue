<template>
  <div class="container">
    <h3 class="text-center">Library Department</h3>
    <table class="table">
      <thead>
        <tr>
      ...
      <tbody>
        <tr v-for="student,key in students">
          ...
          <td v-if="student.librarycleared==1">cleared</td>
          <td v-else>Uncleared</td>
          <td><a type="button" id="show-modal" @click="showModal=true;
            setVal(student.id, student.reg_no, student.name, student.librarycleared)"
    ...
    let vm = this;
     ...
     makePagination(meta, links){
      ...
      updateStudent(){
        ...
        var librarycleared = document.getElementById('status').value
        axios.put(`api/updatelibrarystatus/${id}`,
          {
            reg_no:reg_no,
            name:name,
